# Hola, soy Iván 👋

*Español |[English](README.en.md)*

## Sobre Mí

Soy un desarrollador apasionado por la ciencia de datos, la visualización y el desarrollo de software. Disfruto trabajando en proyectos que combinan el pensamiento analítico con soluciones de software prácticas.

## 🔧 Tecnologías y Herramientas

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![HTML](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)

**Lenguajes:** Python, C++, Git

**Áreas de Experiencia:**
- Visualización y Análisis de Datos
- Análisis Estadístico
- Desarrollo de Dashboards
- Estructuras de Datos y Algoritmos
- Aplicaciones Cliente-Servidor

## 📊 Proyectos Destacados

### 🌿 [Dashboard de Visualización de Roya del Café](https://github.com/Idromerom714/Visualiaciones_roya_cafe)
Dashboard web para visualizaciones y análisis de la roya en los cultivos de café
- **Stack Tecnológico:** Python
- **Enfoque:** Visualización de datos agrícolas, análisis de enfermedades

### 🅿️ [Sistema de Parqueadero](https://github.com/Idromerom714/parqueadero_)
Implementación de un sistema de gestión de parqueadero con interfaces de cliente y servidor
- **Stack Tecnológico:** HTML, Pythn, C++
- **Enfoque:** Arquitectura cliente-servidor, gestión de sistemas

### 📝 [Tutor ICFES](https://github.com/Idromerom714/Tutor-ICFES)
Aplicación de tutoría para preparación al examen ICFES,impulsado por LLMs y generacion aumentada por recuperación (RAG).
- **Stack Tecnológico:** Python
- **Enfoque:** Educación, preparación de exámenes, herramientas de aprendizaje

## 🌱 Actualmente Aprendiendo

- Machine learning
- Modelos grandes de lenguaje (LLMs)
- Técnicas avanzadas de visualización de datos
- Desarrollo de aplicaciones web
- Modelado y análisis estadístico
- Prácticas modernas de C++

## 📫 Cómo Contactarme

¡Siéntete libre de explorar mis repositorios y contactarme si te gustaría colaborar en proyectos relacionados con ciencia de datos, visualización o desarrollo de software!

---

💡 *"Convirtiendo datos en conocimiento, una visualización a la vez"*
